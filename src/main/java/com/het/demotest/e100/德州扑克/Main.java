package com.het.demotest.e100.德州扑克;
...
public class Main {
    ...
    static boolean fun5() {// 顺子 
        if (num.size() != 5) return false;// 判断是不是5个不同大小，不是的话有重复肯定不是顺子
        ...
        for (int i = 0; i < 3; i++) {//前四个不满足递增且相差1
            if (v.get(i) + 1 != v.get(i + 1)) return false;
        }
        if (v.get(3) + 1 == v.get(4)) return true;// 满足第一种顺子
        if (v.get(0) == 2 && v.get(4) == 14) return true;// 满足第二种顺子
        return false;
    }
    ...
    static int solve() {
        // 将牌映射成连续的编号，用于排序
        for (int i = 2; i <= 10; i++) idx.put(String.valueOf(i), i);
        idx.put("J", 11);
        idx.put("Q", 12);
        idx.put("K", 13);
        idx.put("A", 14);
